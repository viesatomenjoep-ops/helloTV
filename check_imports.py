...
for filepath in files_to_check:
    ...
    if lucide_import_match:
        ...
        # Handle "Youtube as YoutubeIcon"
        imported_icons = [i.split(' as ')[-1].strip() for i in imported_icons]
    
    # Find all used icons
    used_icons_matches = re.findall(r"<([A-Z][a-zA-Z0-9]+)\s", content)
    used_components = set(used_icons_matches)
    
    # Basic standard HTML/React components to ignore
    ignore = {'React', 'Fragment', 'SqlTerminal', 'HelloTVLogo', 'NewOrderWidget', 'ExternalLink', 'div', 'span', 'button', 'input', 'textarea', 'form', 'select', 'option', 'h1', 'h2', 'h3', 'p', 'img', 'a', 'code', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'br'}
    
    missing_icons = []
    for comp in used_components:
        if comp not in ignore and comp not in imported_icons and comp not in content:
            # Maybe it's defined locally?
